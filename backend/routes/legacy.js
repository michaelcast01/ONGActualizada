...
const buildBeneficiarioWhere = (query) => {
  ...
  if (query.q) {
    ...
    clauses.push(`(
      b.primer_nombre ILIKE $${params.length}
      OR b.apellidos ILIKE $${params.length}
      OR b.numero_documento ILIKE $${params.length}
      OR b.correo ILIKE $${params.length}
    )`);
  }
...
async function resolveCity(pool, cityId) {
  if (!cityId) {
    return { cod_municipio_divipola: null, cod_departamento_divipola: null };
  }
...
async function getDefaultMissionId(pool) {
  ...
  const created = await pool.query(
    `
      INSERT INTO mision_operativa (nombre_mision, tipo_mision, estado)
      VALUES ($1, 'Jornada', 'En Curso')
      RETURNING id
    `,
...
async function getDefaultDonacionId(pool) {
  const donante = await pool.query(
    `
      INSERT INTO donante (tipo, nombre_completo)
      VALUES ('Anonimo', 'Compatibilidad API antigua')
      RETURNING id
    `
  );
  ...
  const donacion = await pool.query(
    `
      INSERT INTO donacion (donante_id, tipo, valor_estimado)
      VALUES ($1, 'Fisica', 0)
      RETURNING id
    `,
    [donante.rows[0].id]
...
async function getLoteForItem(pool, itemId) {
  ...
  const donacionId = await getDefaultDonacionId(pool);
  const created = await pool.query(
    `
      INSERT INTO lote_inventario (item_id, donacion_id, numero_lote, cantidad_inicial, stock_actual)
      VALUES ($1, $2, 'LEGACY', 999999, 999999)
      RETURNING id
    `,
    [itemId, donacionId]
  );
...
function legacyRoutes(pool, authenticateToken) {
  ...
  router.get('/catalogos/tipos-ayuda', asyncHandler(async (req, res) => {
    const result = await pool.query(`
      SELECT id AS id_tipo_ayuda,
             nombre AS nombre_ayuda,
  ...
  router.get('/beneficiarios', asyncHandler(async (req, res) => {
    ...
    const countResult = await pool.query(
      `${beneficiarioCtes} SELECT COUNT(*)::int AS total FROM (${beneficiarioSelect} ${whereSql}) filtered`,
      params
    );
  ...
  router.get('/beneficiarios/:id', asyncHandler(async (req, res) => {
    const result = await pool.query(
      `${beneficiarioCtes} ${beneficiarioSelect} WHERE b.id = $1`,
      [req.params.id]
    );
  ...
  router.post('/beneficiarios', authenticateToken, asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      `
        INSERT INTO beneficiario (tipo_documento, numero_documento, primer_nombre, apellidos, telefono_principal, correo, grupo_sisben, consentimiento_datos)
        VALUES ('CC', $1, $2, $3, $4, $5, $6, true)
        RETURNING id
      `,
    ...
    if (direccion) {
      await pool.query(
        `
          INSERT INTO direccion_ubicacion (beneficiario_id, cod_departamento_divipola, cod_municipio_divipola, direccion_fisica)
          VALUES ($1, $2, $3, $4)
        `,
  ...
  router.put('/beneficiarios/:id', authenticateToken, asyncHandler(async (req, res) => {
    ...
    await pool.query(
      `
        ...
        SET primer_nombre = COALESCE($1, primer_nombre),
            ...
            grupo_sisben = COALESCE($6, grupo_sisben)
        WHERE id = $7
      `,
    ...
    if (direccion || id_ciudad) {
      ...
      await pool.query(
        `
          INSERT INTO direccion_ubicacion (beneficiario_id, cod_departamento_divipola, cod_municipio_divipola, direccion_fisica)
          VALUES ($1, $2, $3, COALESCE($4, 'Sin direccion'))
          ON CONFLICT DO NOTHING
        `,
        [req.params.id, city.cod_departamento_divipola, city.cod_municipio_divipola, direccion || null]
      );
      await pool.query(
        `
          UPDATE direccion_ubicacion
          SET direccion_fisica = COALESCE($1, direccion_fisica),
              cod_municipio_divipola = COALESCE($2, cod_municipio_divipola),
              cod_departamento_divipola = COALESCE($3, cod_departamento_divipola)
          WHERE id = (
            SELECT id FROM direccion_ubicacion WHERE beneficiario_id = $4 ORDER BY id LIMIT 1
          )
        `,
        [direccion || null, city.cod_municipio_divipola, city.cod_departamento_divipola, req.params.id]
      );
    }
  ...
  router.post('/colaboradores', authenticateToken, asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      `
        INSERT INTO usuario (nombre_usuario, correo_electronico, hash_contrasena, esta_activo)
        VALUES ($1, $2, $3, true)
        RETURNING id
      `,
      [nombre_colaborador, correo || `colaborador_${cedula}@ong.local`, hash]
    );
  ...
  router.put('/colaboradores/:id', authenticateToken, asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      `
        ...
        RETURNING id
      `,
      [nombre_colaborador || null, correo || null, req.params.id]
    );
  ...
  const buildEntregaWhere = (query) => {
    ...
    if (query.q) {
      params.push(`%${String(query.q).trim()}%`);
      clauses.push(`(b.primer_nombre ILIKE $${params.length} OR b.apellidos ILIKE $${params.length} OR b.numero_documento ILIKE $${params.length} OR ii.nombre ILIKE $${params.length})`);
    }
    if (query.cityId) {
      params.push(String(query.cityId));
      clauses.push(`(c.id_ciudad::text = $${params.length} OR du.cod_municipio_divipola = $${params.length})`);
  ...
  router.get('/entregas', asyncHandler(async (req, res) => {
    ...
    const dataResult = await pool.query(
      `${entregaQuery} ${whereSql} ORDER BY ee.fecha_entrega DESC, ee.id DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, limit, offset]
    );
  ...
  router.post('/entregas', authenticateToken, asyncHandler(async (req, res) => {
    ...
    const entrega = await pool.query(
      `
        INSERT INTO entrega_encabezado (beneficiario_id, mision_id, usuario_id, fecha_entrega, estado)
        VALUES ($1, $2, $3, $4, 'Completada')
        RETURNING id
      `,
      [id_beneficiario, misionId, id_colaborador, fecha_entrega]
    );

    await pool.query(
      `
        INSERT INTO entrega_detalle (entrega_encabezado_id, lote_inventario_id, cantidad_entregada)
        VALUES ($1, $2, $3)
      `,
      [entrega.rows[0].id, loteId, cantidad]
    );
  ...
  router.put('/entregas/:id', authenticateToken, asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      `
        ...
        RETURNING id
      `,
      [fecha_entrega || null, id_beneficiario || null, id_colaborador || null, observaciones || null, req.params.id]
    );
    ...
    if (id_tipo_ayuda || cantidad) {
      ...
      await pool.query(
        `
          ...
          SET lote_inventario_id = COALESCE($1, lote_inventario_id),
              cantidad_entregada = COALESCE($2, cantidad_entregada)
          WHERE id = (
            SELECT id FROM entrega_detalle WHERE entrega_encabezado_id = $3 ORDER BY id LIMIT 1
          )
        `,
        [loteId, cantidad || null, req.params.id]
      );
    }
